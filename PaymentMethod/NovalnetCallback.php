...
class NovalnetCallback
{
     ...
     * @return null
     */
    public function startProcess($event, $logger, $paymentProvider)
    {
        ...
        $this->novalnetConfig = $paymentMethod->config;
    ...
    public function validateCaptureParams($requestParam)
    {
        ...
        if ($callerIp != $realHostIp && !$this->novalnetConfig->getCallbackTestmode()) {
    ...
    protected function sendNotifyMail($datas, $missingTransNotify = false)
    {
        ...
        } elseif ($this->novalnetConfig->getEmailNotification() && !empty($this->novalnetConfig->getEmailTo())) {
            $email->setTo(explode(',', $this->novalnetConfig->getEmailTo()));
            if (!empty($this->novalnetConfig->getEmailBcc())) {
                $email->setBcc(explode(',', $this->novalnetConfig->getEmailBcc()));
            }
