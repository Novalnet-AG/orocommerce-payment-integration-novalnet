<?php

namespace Novalnet\Bundle\NovalnetBundle\PaymentMethod\Config;

/**
 * Configuration class which is used to get specific configuration for Novalnet payment method
 * Usually it has additional get methods for payment type specific configurations
 */
class NovalnetTrustlyConfig extends NovalnetConfig implements NovalnetTrustlyConfigInterface
{
    const ADMIN_LABEL_KEY = 'Trustly';
    const ADMIN_LABEL_SUFFIX = 'TRUSTLY';
    const TRUSTLY_TESTMODE = 'trustly_testmode';
    const TRUSTLY_NOTIFICATION = 'trustly_buyer_notification';

    /**
     * {@inheritDoc}
     */
    public function getTestMode()
    {
        return (int)$this->get(self::TRUSTLY_TESTMODE);
    }

    /**
     * {@inheritDoc}
     */
    public function getBuyerNotification()
    {
        return (string)$this->get(self::TRUSTLY_NOTIFICATION);
    }
}
