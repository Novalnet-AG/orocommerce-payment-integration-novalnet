...
class NovalnetResponseListener
{
    ...
    public function onError(AbstractCallbackEvent $event)
    {
        ...
        $qryBuilder = $this->doctrine->getManager();
        $novalnetConfig = $this->novalnetHelper->getNovalnetConfig($this->doctrine);
        $paymentTransaction
            ->setSuccessful(false)
            ->setActive(false);
        ...
        if (isset($eventData['tid'])) {         
            ...
            $this->novalnetHelper->insertNovalnetTransactionTable($this->doctrine, $this->doctrineHelper, $eventData);
    ...
    public function onReturn(AbstractCallbackEvent $event)
    {
        ...
        if (isset($eventData['hash2'])) {
            $currentHash = $this->novalnetHelper->generateHash($eventData, $novalnetConfig['payment_access_key']);
    ...
    public function onNotify(AbstractCallbackEvent $event)
    {
        ...
        $eventData = $event->getData();
        $paymentTransaction = $event->getPaymentTransaction();
        $this->novalnetCallback->startProcess($event, $this->logger);
